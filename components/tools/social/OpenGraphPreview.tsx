...
export default function OpenGraphPreview() {
	...
	const InstagramPreview = () => (
		<div className="bg-card border border-border p-6 rounded-xl max-w-[400px] mx-auto font-sans">
			<div className="flex items-center gap-2 mb-3">
				<div className="w-8 h-8 rounded-full bg-muted"></div>
				<div className="text-sm font-semibold text-foreground">instagram_user</div>
			</div>
			...
			<div className="flex justify-end">
				<div className="bg-[#efefef] rounded-2xl p-4 max-w-[280px] overflow-hidden">
					...
					<h4 className="font-semibold text-sm leading-tight mb-1 line-clamp-2 text-foreground">{data.title}</h4>
					<p className="text-xs text-muted-foreground line-clamp-1">{data.description}</p>
					<p className="text-[10px] text-muted-foreground mt-2 uppercase">{new URL(data.url).hostname}</p>
				</div>
			</div>
		</div>
	);
		...
		<div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)] min-h-[600px]">
			...
			<div className="w-full lg:w-[400px] flex flex-col bg-card border border-border rounded-xl shadow-sm overflow-hidden flex-shrink-0">
				...
				<div className="flex-1 overflow-y-auto p-5 space-y-6">
					{/* URL Input */}
					<div>
						<label className="text-xs font-bold text-muted-foreground uppercase mb-2 block">Website URL</label>
						<div className="flex gap-2">
							<div className="relative flex-1">
								...
								<input
									...
									className="w-full pl-9 pr-3 py-2.5 border border-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all bg-background text-foreground"
								/>
							</div>
							<button
								onClick={handleFetch}
								disabled={loading}
								className="px-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 disabled:opacity-50 transition-colors"
							>
					...
					<div className="space-y-4">
						<h4 className="text-sm font-bold text-foreground">Manual Override</h4>
						...
						<div>
							...
							<input
								...
								className="w-full px-3 py-2 border border-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-background text-foreground"
							/>
						</div>
						...
						<div>
							...
							<textarea
								...
								rows={3}
								className="w-full px-3 py-2 border border-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none bg-background text-foreground"
							/>
						</div>

						<div>
							<label className="text-xs font-bold text-muted-foreground uppercase mb-1.5 block">Image URL</label>
							<div className="flex gap-2">
								<input
									type="text"
									value={data.image}
									onChange={(e) => handleManualChange("image", e.target.value)}
									className="flex-1 px-3 py-2 border border-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-background text-foreground"
								/>
							</div>
						</div>
					</div>
					<div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 mt-6">
						<div className="flex items-start gap-2 text-yellow-600 dark:text-yellow-500 mb-2">
			...
			<div className="flex-1 flex flex-col min-w-0 bg-card border border-border rounded-xl shadow-sm overflow-hidden">
				...
				{/* Tabs */}
				<div className="border-b border-border bg-muted/50 p-2 overflow-x-auto">
				...
				{/* Canvas */}
				<div className="flex-1 bg-muted/30 p-8 overflow-y-auto flex items-center justify-center relative">
				...
				{/* Footer Info */}
				<div className="bg-card p-3 border-t border-border text-xs text-muted-foreground text-center">
