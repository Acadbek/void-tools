    ...
    <div className="flex flex-col h-full gap-4">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-muted/30 p-3 rounded-xl border border-border">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <span className="text-sm font-medium text-foreground whitespace-nowrap">Interface Name:</span>
          <input
            type="text"
            value={rootName}
            onChange={(e) => setRootName(e.target.value)}
            className="px-2 py-1 text-sm border rounded bg-card focus:ring-2 focus:ring-blue-500 outline-none w-full sm:w-40 text-foreground"
          />
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          <button
            ...
            className="text-xs text-destructive hover:bg-destructive/10 px-3 py-1.5 rounded transition-colors flex items-center gap-1.5 font-medium border border-transparent hover:border-destructive/20"
          >
      ...
      <div className="grid md:grid-cols-2 gap-4 h-[500px] lg:h-[600px]">
        <div className="flex flex-col border border-border rounded-xl overflow-hidden bg-card shadow-sm hover:shadow-md transition-shadow">
          ...
          <textarea
            ...
            className={`flex-1 w-full p-4 outline-none font-mono text-sm resize-none bg-card text-foreground ${error ? "bg-destructive/10" : ""
              }`}
            spellCheck={false}
          />
        </div>

        <div className="flex flex-col border border-border rounded-xl overflow-hidden bg-[#1e1e1e] shadow-sm">
      ...
      <div className="md:hidden text-center text-xs text-muted-foreground flex items-center justify-center gap-1">
