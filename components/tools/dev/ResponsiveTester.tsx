...
export default function ResponsiveTester() {
	...
	return (
		<div className="flex flex-col h-[calc(100vh-140px)] min-h-[600px] gap-4">
			...
			<div className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm flex flex-col gap-4 shrink-0">
				...
				<form onSubmit={handleSubmit} className="flex gap-2 w-full">
					<div className="relative flex-1">
						...
						<input
							...
							className="w-full pl-9 pr-3 py-2 border text-gray-700 dark:text-gray-200 border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50 focus:bg-white transition-colors"
						/>
					</div>
					<button
						type="submit"
				...
				<div className="flex flex-wrap items-center justify-between gap-3">
					...
					{/* Devices List */}
					<div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-thin scrollbar-thumb-gray-200 w-full md:w-auto">
						{DEVICES.map((device) => (
							<button
								key={device.name}
                 ...
                 `}
					...
					<div className="flex items-center gap-3 ml-auto">
						...
						<button
							...
							className="p-1.5 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg disabled:opacity-30 disabled:hover:bg-transparent transition-colors border border-transparent hover:border-blue-100"
							title="Rotate Screen"
						>
						...
						<div className="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white">
							<button
								...
								className="p-1.5 hover:bg-gray-50 text-gray-500 active:bg-gray-100"
								title="Zoom Out"
							>
								<ZoomOut className="w-3.5 h-3.5" />
							</button>
							<span className="text-[10px] font-mono w-9 text-center text-gray-600 font-medium select-none">
								{Math.round(zoom * 100)}%
							</span>
							<button
			...
			{/* --- PREVIEW AREA --- */}
			<div className="flex-1 bg-gray-100 border border-gray-200 rounded-xl overflow-hidden relative flex flex-col shadow-inner">

				{/* Info Bar */}
				<div className="bg-white border-b border-gray-200 px-4 py-2 flex justify-between items-center text-xs text-gray-500 shrink-0 z-10">
					<span className="flex items-center gap-2">
						<span className={`w-2 h-2 rounded-full ${activeUrl ? 'bg-green-500' : 'bg-gray-300'}`}></span>
						Device: <strong className="text-gray-800">{selectedDevice.name}</strong>
						{typeof dims.width === 'number' && ` (${dims.width} x ${dims.height})`}
					</span>
					{activeUrl && (
						<span className="truncate max-w-[200px] bg-gray-50 px-2 py-0.5 rounded border border-gray-100">
							{activeUrl}
						</span>
					)}
				</div>
					...
					{activeUrl ? (
						<div className="overflow-auto w-full h-full flex items-center justify-center p-8">
							<div
								...
								className="bg-white shadow-2xl transition-all duration-300 relative shrink-0 border border-gray-300"
							>
								<iframe
									src={activeUrl}
						...
						<div className="flex flex-col items-center justify-center text-gray-400 max-w-md text-center p-6">
							<Monitor className="w-16 h-16 mb-4 opacity-20" />
							<h3 className="font-bold text-gray-700 text-lg">Enter a URL to start testing</h3>
							<p className="text-sm mt-2 text-gray-500">
								Type a website address above and click Test.
								<br />
								Try inputs like <code className="bg-gray-200 px-1 rounded text-gray-700">localhost:3000</code>
							</p>
						</div>
					)}
				</div>
				...
				{/* Warning Footer */}
				<div className="bg-amber-50 border-t border-amber-100 p-2 flex items-center justify-center gap-2 text-[10px] sm:text-[11px] text-amber-700 shrink-0">
