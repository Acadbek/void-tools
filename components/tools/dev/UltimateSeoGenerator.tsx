			...
			<div className="max-w-7xl mx-auto">
				<div className="flex flex-col lg:flex-row gap-6 min-h-[600px]">
					...
					<div className="w-full lg:w-64 flex flex-col gap-1.5 shrink-0">
						<div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 px-2">Core SEO</div>
						<button
						...
						<div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 px-2 mt-4">Visual & Rich</div>
						<button
						...
						<div className="bg-white dark:bg-gray-900 p-4 md:p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm transition-colors">
							...
							{activeTab === 'meta' && (
								<div className="space-y-4 animate-in fade-in">
									<h3 className="font-bold flex gap-2 text-gray-900 dark:text-white"><Globe className="w-5 h-5 text-blue-500" /> Metadata</h3>
									...
									<div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
										{faqItems.map((item, index) => (
											<div key={index} className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg border border-gray-100 dark:border-gray-700 relative group">
												...
												/>
												{faqItems.length > 1 && (
													<button onClick={() => removeFaq(index)} className="absolute top-2 right-2 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition">
							...
							{activeTab === 'robots' && (
								<div className="animate-in fade-in">
									<h3 className="font-bold text-gray-900 dark:text-white">Robots.txt</h3>
									<p className="text-sm text-gray-500 dark:text-gray-400">Standard allow all configuration.</p>
								</div>
							)}
							...
							{activeTab === 'og-image' && (
								<div className="space-y-6 animate-in fade-in">
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
										...
										<div className="space-y-3">
											...
											<div className="flex gap-2">
												...
												<label className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-400 cursor-pointer border dark:border-gray-700 px-2 rounded bg-gray-50 dark:bg-gray-800">
										...
										<div className="space-y-3">
											...
											<div className="flex items-center gap-4 p-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800">
												<select value={ogConfig.bgType} onChange={(e: any) => setOgConfig({ ...ogConfig, bgType: e.target.value })} className="text-sm border dark:border-gray-600 rounded p-1 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
												...
												<div className="flex items-center gap-1">
													...
													{ogConfig.bgType === 'gradient' && <span className="text-gray-400">→</span>}
											...
											<div className="flex gap-4 justify-between">
												<div><div className="text-[10px] uppercase font-bold text-gray-400 mb-1">Text</div><input type="color" value={ogConfig.textColor} onChange={(e) => setOgConfig({ ...ogConfig, textColor: e.target.value })} className={STYLES.colorInput} /></div>
												<div><div className="text-[10px] uppercase font-bold text-gray-400 mb-1">Accent</div><input type="color" value={ogConfig.accentColor} onChange={(e) => setOgConfig({ ...ogConfig, accentColor: e.target.value })} className={STYLES.colorInput} /></div>
											</div>
										</div>

										{/* Typography & Layout */}
										<div className="space-y-3 md:col-span-2">
											<div className={`${STYLES.controlGroupLabel} border-t border-gray-100 dark:border-gray-800 pt-3`}><Layout className="w-3.5 h-3.5" /> Typography & Layout</div>
											<div className="flex flex-wrap items-center gap-4">
												<div className="flex rounded-md bg-gray-100 dark:bg-gray-800 p-1 border dark:border-gray-700">
												...
												<select value={ogConfig.titleFontWeight} onChange={(e) => setOgConfig({ ...ogConfig, titleFontWeight: e.target.value })} className="text-sm border border-gray-200 dark:border-gray-700 rounded p-1.5 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
													<option value="400">Normal</option> <option value="700">Bold</option> <option value="900">Black</option>
												</select>
												<div className="flex-1 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
									...
									{/* LIVE PREVIEW */}
									<div className="mt-6">
										<div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Live Preview (1200 x 630)</div>
										<div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-lg aspect-[1200/630] w-full mx-auto relative transition-all duration-300"
											style={{
